...
#     raise EnvironmentError("Set PRIVATE_KEY and RPC_URL in .env.local")
...
#     def transfer_token(self, token_contract, to, amount):
#         try:
#             abi = load_abi("contracts/TokenABI.json")
#             contract = self.w3.eth.contract(address=Web3.to_checksum_address(token_contract), abi=abi)
...
#             tx_hash = self.w3.eth.send_raw_transaction(signed_tx.rawTransaction)
#             logging.info(f"Transferred {amount} tokens to {to}, tx hash: {tx_hash.hex()}")
#             return tx_hash.hex()
#         except Exception as e:
